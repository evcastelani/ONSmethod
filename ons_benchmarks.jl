...
f=open("results_ons.txt","w")
for prob in list1
    global A=readdlm("examples/$(prob)");
    xsol,countsol,valsol=ONSNewton(model,A,3,25,0.25)
    println(f,prob)
    println(f,norm(abs.(xsol)-sol,Inf), "  ", countsol, "  ",valsol)
    b= @benchmark ONSNewton(model,A,3,25,0.25)
...
for prob in list2
    global A=readdlm("examples/$(prob)");
    xsol,countsol,valsol=ONSNewton(model,A,3,50,0.25)
    println(f,prob)
    println(f,norm(abs.(xsol)-sol,Inf), "  ", countsol, "  ",valsol)
    b= @benchmark ONSNewton(model,A,3,50,0.25)
    println(f,(10.0^(-9))*median(b).time," s  ",(10.0^(-6))*(median(b).memory/1024)," Mb ")
    println(f,"**********************************************")
end
